...
詳細は[リポジトリの優先度](articles/repository-priorities.md)の記事を読んでこの挙動を変える方法を見てください。
...
`providers-api`フィールドでは与えられたパッケージ名を提供する全てのパッケージ、ただしその名前のパッケージ以外、を返すURLテンプレートを与えられます。空欄`%package%`を含まなければなりません。
...
リポジトリのルートからの絶対パスです。`%package%`と`%hash%`の空欄を含まなければいけません。
...
上のファイルはこのリポジトリにacme/fooとacme/barがあることを宣言しています。
`providers-url`で参照されたファイルを読み込み、ベンダーの名前空間が付いたパッケージ名で`%package%`で、`%hash%`をsha256フィールドで、それぞれ置き換えます。
