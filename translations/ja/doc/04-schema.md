...
> **補足：**
> この機能には厳しい技術的な制約があります。
> 参照はチェックアウトされるコミットがどれかを変えるのみであって、
> 低水準でのオーバーライドとしてインストール時に適用され、そのオーバーライドは依存関係の解決器に触れることのないものです。
> 結果として、次のようになります。
>
> - パッケージの`composer.json`のメタ情報（自身の`require`項目、自動読み込みの規則、などなど）は
>   名付けたブランチから読まれ、それの現在の状態においてであり、
>   ピン留めしたコミットからではありません。
>   Composerが解決した依存関係は、したがって、そのコミットに宣言されたものとは違うものになりえます。
> - オーバーライドはソースのインストールに対してのみ信頼できます。
>   distのインストールは、任意のコミットに対するアーカイブを構築できる（例：GitHub）ソースからdistが取得されたときにその参照を尊重できるのみであって、
>   そうでなければファイルをダウンロードするときにピン留めされたコミットは無視されます。
>
> したがってこれは、タグ付けされたリリースに切り替えられるまで、一過的な問題を和らげるための開発中の一時的な解決策としてのみ使うべきです。
> Composerのチームはこの機能へ活発に対応しておらず関係する不具合の報告を受け付けません。
...
しかしJSONのキーバリュー対は順序なしとして見做されるため、一貫した挙動は保証されず、廃止されました。
